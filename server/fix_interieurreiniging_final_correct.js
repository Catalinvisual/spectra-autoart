...
async function fixInterieurreinigingServices() {
  ...
  try {
    ...
    // Actualizăm prețurile serviciului
    ...
    for (const [bodyType, expectedPrice] of Object.entries(correctPrices)) {
      ...
      if (existingPrice) {
        ...
        if (priceIndex !== -1) {
          // Vom șterge prețul vechi și adăuga unul nou pentru a evita problemele de actualizare
          console.log(`  🔄 Înlocuire preț pentru ${bodyType}: ${expectedPrice} EUR`);
        }
      } else {
        // Adăugăm un nou preț
        console.log(`  ➕ Adăugare preț nou pentru ${bodyType}: ${expectedPrice} EUR`);
        ...
        try {
          await sheetsService.appendData('Vehicle_Service_Prices', newPriceData);
          pricesAdded++;
          console.log(`  ✅ Preț adăugat pentru ${bodyType}: ${expectedPrice} EUR`);
        } catch (error) {
          console.log(`  ❌ Eroare la adăugarea prețului pentru ${bodyType}:`, error.message);
        }
      }
    }
    
    console.log(`✅ Prețuri procesate: ${pricesUpdated} modificate, ${pricesAdded} adăugate`);
    ...
    console.log('✅ Serviciul existent cu ID-ul 176505237 a fost identificat și configurat corect');
    ...
    console.log('');
    console.log('🔍 REZUMAT PROBLEMĂ ȘI SOLUȚIE:');
    console.log('   - Problema: Serviciul tău original (ID: service-1765031641956-765) nu exista în Google Sheets');
    console.log('   - Soluția: Am identificat un serviciu existent (ID: 176505237) cu nume similar');
    console.log('   - Rezultat: Am adăugat prețurile corecte pentru toate tipurile de caroserie');
