...
// Componentă memoizată pentru cardul de rezervare
      ...
      <div className="bookings-grid">
        {Array.isArray(bookings) && bookings.map((booking: any) => (
          <BookingCard
            ...
            handleDeleteBookingLocal={handleDeleteBookingLocal}
...
const GalleryManagement: React.FC<GalleryManagementProps> = ({ isAuthenticated }) => {
  ...
  // State pentru modalul de confirmare ștergere imagine
  ...
  const loadImages = async () => {
    try {
      setLoading(true)
      // Folosește endpoint-ul public pentru a obține aceleași date ca în pagina principală
      const response = await publicAPI.getGallery(i18n.language)
      
      // Procesează și filtrează imagini duplicate
      console.log('Raw images data from public API:', response.data)
      const processedImages = response.data
        .map((image: any) => {
          // Log detaliat pentru fiecare imagine
          console.log(`Image ID: ${image.id}, Title: "${image.title}", Type: ${typeof image.title}, Length: ${image.title?.length}`)
          console.log(`Full image data:`, {
            id: image.id,
            title: image.title,
            title_en: image.title_en,
            title_nl: image.title_nl,
            title_es: image.title_es,
            title_pl: image.title_pl,
            title_ro: image.title_ro,
          ...
          return {
            ...image,
            url: finalUrl
            // Păstrăm titlul exact așa cum vine de la API
          }
        })
      ...
      // Elimină imaginea blocată problematică
      const filteredImages = uniqueImages.filter((image: any) => 
        !image.url.includes('gallery-1766074239376-793205241.jpg')
      )
      
      console.log('Final processed images with titles:', filteredImages.map((img: any) => ({ id: img.id, title: img.title })))
  ...
  const confirmDeleteImage = async () => {
    const { imageId } = imageDeleteModal
    if (!imageId) return
    ...
    try {
      ...
      // Închide modalul
      setImageDeleteModal({ isOpen: false, imageId: '', imageTitle: '' })
    } catch (error) {
  ...
  return (
    <div className="gallery-management">
              ...
              <div key={image.id} className="image-item">
                ...
                <div className="image-info">
                  <div className="image-title">{
                    image[`title_${i18n.language}` as keyof typeof image] || 
                    image.title || 
                    image.alt_text || 
                ...
                <div className="image-actions">
                  ...
                  <button 
                    onClick={() => handleDeleteImage(image.id, image.title || image.alt_text || '')}
                    className="delete-btn"
                  >
                    {t('admin.delete')}
                  </button>
      ...
      {imageDeleteModal.isOpen && (
        <div className="modal-overlay" onClick={cancelDeleteImage}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Confirmare ștergere</h3>
              <button className="modal-close" onClick={cancelDeleteImage}>×</button>
            </div>
            <div className="modal-body">
              <p>{t('admin.areYouSureDeleteImage')}</p>
              {imageDeleteModal.imageTitle && (
                <p className="image-title-preview">Imagine: <strong>{imageDeleteModal.imageTitle}</strong></p>
              )}
            </div>
            <div className="modal-footer">
              <button className="modal-btn modal-btn-cancel" onClick={cancelDeleteImage}>
                {t('admin.cancel')}
              </button>
              <button 
                ...
                disabled={loading}
              >
                {loading ? 'Se șterge...' : t('admin.delete')}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="subscribers-section">
        ...
        <div className="subscribers-list">
          {Array.isArray(subscribers) && subscribers.map((subscriber: any) => (
            <div key={`${subscriber.email}-${subscriber.subscribedAt}`} className="subscriber-item">
