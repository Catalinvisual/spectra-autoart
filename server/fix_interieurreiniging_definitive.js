...
async function fixInterieurreinigingServices() {
  ...
  try {
    ...
    // Identificăm serviciul "Interieurreiniging Premium" existent
    console.log('\n🔍 Căutare serviciu "Interieurreiniging Premium"...');
    
    const targetService = servicesData.find(service => {
      const serviceId = service[0]; // ID este pe prima coloană
      const serviceNameNL = String(service[2] || service[1] || '').toLowerCase().trim(); // Name_NL este pe coloana 3, Name pe 2
      const serviceNameEN = String(service[1] || '').toLowerCase().trim();
      
      return serviceNameNL.includes('interieurreiniging premium') || 
             serviceNameEN.includes('premium interior cleaning') ||
             serviceNameNL.includes('eersteklas interieurreiniging');
    });
    
    if (!targetService) {
      console.log('❌ Nu a fost găsit serviciul "Interieurreiniging Premium"');
      return;
    }
