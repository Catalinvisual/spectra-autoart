...
import { BODY_TYPES } from './server/src/config/bodyTypesConfig.js';

// Simulează funcția mapFrontendKeyToBodyType din vehicleServicesService.js
function mapFrontendKeyToBodyType(frontendKey, bodyTypes) {
  ...
  const bodyType = bodyTypes.find(bt => bt.key === mappedKey);
  if (!bodyType) {
...
const activeBodyTypes = BODY_TYPES.filter(bt => bt.is_active);
...
Object.keys(inputPrices).forEach(frontendKey => {
  const mappedBodyType = mapFrontendKeyToBodyType(frontendKey, activeBodyTypes);
  if (mappedBodyType) {
