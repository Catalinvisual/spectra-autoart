...
async function populateVehiclesSheet() {
  try {
    ...
    // Inițializăm Google Sheets Service
    console.log('Inițializare Google Sheets Service...')
    ...
    // Verificăm dacă foaia are antet
    const existingData = await GoogleSheetsService.getData('Vehicles')
    if (existingData.length === 0) {
      // Adăugăm antetul
      const headerRow = [
        'ID', 'Make', 'Model', 'Type', 'Body', 'Active', 'Created_Date', 'Updated_Date'
      ]
      await GoogleSheetsService.appendData('Vehicles', [headerRow])
      console.log('✅ Antet adăugat în Google Sheets')
    }
