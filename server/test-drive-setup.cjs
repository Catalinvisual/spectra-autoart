const { google } = require('googleapis');

async function checkGoogleDriveSetup() {
  try {
    console.log('🔍 Verificare configurare Google Drive...');
    
    // Autentificare cu service account
    const auth = new google.auth.GoogleAuth({
      credentials: {
        client_email: 'spectra-autoart@spectra-autoart.iam.gserviceaccount.com',
        private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
      },
      scopes: ['https://www.googleapis.com/auth/drive.file']
    });

    const drive = google.drive({ version: 'v3', auth });
    
    console.log('✅ Autentificare Google reușită');
    
    // Verificare spațiu disponibil
    console.log('\n📊 Verificare spațiu de stocare...');
    try {
      const about = await drive.about.get({ fields: 'storageQuota' });
      console.log('📈 Spațiu total:', Math.round(about.data.storageQuota.limit / (1024*1024*1024)), 'GB');
      console.log('📊 Spațiu folosit:', Math.round(about.data.storageQuota.usage / (1024*1024*1024)), 'GB');
      console.log('✅ Spațiu disponibil:', Math.round((about.data.storageQuota.limit - about.data.storageQuota.usage) / (1024*1024*1024)), 'GB');
    } catch (error) {
      console.log('⚠️ Nu pot accesa informațiile despre spațiu:', error.message);
    }
    
    // Verificare folder
    console.log('\n📁 Verificare folder Drive...');
    try {
      const folder = await drive.files.get({
        fileId: '1cLJwq0c5Dv3wOuTQTy06a27-IV9O_kaM',
        fields: 'id,name,parents,shared,permissions'
      });
      
      console.log('✅ Folder găsit:', folder.data.name);
      console.log('📂 ID:', folder.data.id);
      console.log('🔗 Este shared:', folder.data.shared);
      
      // Verificare permisiuni
      if (folder.data.permissions) {
        console.log('🔑 Permisiuni:', folder.data.permissions.length);
        folder.data.permissions.forEach(perm => {
          console.log(`   - ${perm.role} pentru ${perm.type}`);
        });
      }
      
    } catch (error) {
      console.log('❌ Folderul nu a fost găsit sau nu ai acces:', error.message);
    }
    
    // Test upload fișier mic
    console.log('\n📤 Test upload fișier...');
    try {
      const testContent = 'Test file content';
      const fileMetadata = {
        name: 'test-upload.txt',
        parents: ['1cLJwq0c5Dv3wOuTQTy06a27-IV9O_kaM']
      };
      const media = {
        mimeType: 'text/plain',
        body: Buffer.from(testContent)
      };
      
      const file = await drive.files.create({
        requestBody: fileMetadata,
        media: media,
        fields: 'id,name,webViewLink,webContentLink'
      });
      
      console.log('✅ Upload reușit!');
      console.log('📄 Fișier ID:', file.data.id);
      console.log('🔗 Link vizualizare:', file.data.webViewLink);
      console.log('🔗 Link download:', file.data.webContentLink);
      
      // Ștergere fișier test
      await drive.files.delete({ fileId: file.data.id });
      console.log('🗑️ Fișier test șters');
      
    } catch (error) {
      console.log('❌ Upload eșuat:', error.message);
      if (error.code === 403) {
        console.log('🔒 Eroare de permisiuni - verifică sharing-ul folderului');
      } else if (error.code === 404) {
        console.log('📁 Folderul nu există');
      }
    }
    
  } catch (error) {
    console.error('❌ Eroare:', error.message);
  }
}

checkGoogleDriveSetup();