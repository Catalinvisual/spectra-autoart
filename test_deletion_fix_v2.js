// Test script to verify the deletion fix - corrected version
...
async function testDeletion() {
  try {
    ...
    console.log('📊 Gallery response structure:', Object.keys(galleryResponse.data))
    
    // Check if it's an array or has a data property
    const images = Array.isArray(galleryResponse.data) ? galleryResponse.data : galleryResponse.data.data || []
    ...
    const remainingImages = Array.isArray(verifyResponse.data) ? verifyResponse.data : verifyResponse.data.data || []
    ...
    if (error.response?.status === 401) {
      console.log('🔑 Authentication required - trying without auth header...')
      // Try without auth to see what happens
      try {
        const galleryResponse = await axios.get('http://localhost:8080/api/public/gallery')
        console.log('📸 Gallery response (no auth):', galleryResponse.data)
      } catch (authError) {
        console.error('❌ Still failed:', authError.response?.data || authError.message)
      }
    }
  }
}
